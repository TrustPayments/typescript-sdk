...
describe('TransactionService', () => {
        ...
        it('count successful', () => {
            ...
            transactionService.create(config.space_id, getTransactionPayload())
                .then((response: { response: http.IncomingMessage, body: TrustPayments.model.Transaction }) => {
                    ...
                    let entityQueryFilter: TrustPayments.model.EntityQueryFilter = new TrustPayments.model.EntityQueryFilter();
                    entityQueryFilter.fieldName = 'id';
                    entityQueryFilter.value = transaction.id;
                    entityQueryFilter.type = TrustPayments.model.EntityQueryFilterType.LEAF;
                    entityQueryFilter.operator = TrustPayments.model.CriteriaOperator.EQUALS;
    ...
    describe('create', () => {
        it('create success', () => {
            transactionService.create(config.space_id, getTransactionPayload())
                .done((response: { response: http.IncomingMessage, body: TrustPayments.model.Transaction }) => {
                    let transaction: TrustPayments.model.Transaction = response.body;
                    expect(transaction.state).to.equal(TrustPayments.model.TransactionState.PENDING);
                });
        });
    });
    ...
    describe('fetchPaymentMethods', () => {
        it('fetchPaymentMethods successful', () => {
            ...
            .done((response: { response: http.IncomingMessage, body: Array<TrustPayments.model.PaymentMethodConfiguration> }) => {
                let paymentMethods: Array<TrustPayments.model.PaymentMethodConfiguration> = response.body;
        ...
        it('search success', () => {
            ...
            transactionService.create(config.space_id, getTransactionPayload())
                .then((response: { response: http.IncomingMessage, body: TrustPayments.model.Transaction }) => {
                    ...
                    entityQueryFilter.type = TrustPayments.model.EntityQueryFilterType.LEAF;
                    entityQueryFilter.operator = TrustPayments.model.CriteriaOperator.EQUALS;
                    let entityQuery = new TrustPayments.model.EntityQuery();
                    entityQuery.filter = entityQueryFilter;
                    return transactionService.search(config.space_id, entityQuery);
                })
                .then((response: { response: http.IncomingMessage, body: Array<TrustPayments.model.Transaction> }) => {
                    let transactionSearch: Array<TrustPayments.model.Transaction> = response.body;
    ...
    describe('update', () => {
        it('update success',  () => {
            transactionService.create(config.space_id, getTransactionPayload())
                .then((response: { response: http.IncomingMessage, body: TrustPayments.model.Transaction }) => {
                    let transaction: TrustPayments.model.Transaction = response.body;
                    let transactionPending: TrustPayments.model.TransactionPending = new TrustPayments.model.TransactionPending();
